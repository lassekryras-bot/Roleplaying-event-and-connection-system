...
function createHundredNotesTestProject(): ProjectBoardData {
  const random = createSeededRandom(1005);
  const timelineAnchors: BoardTimelineAnchor[] = ['past', 'now', 'future_possible'];
  const patterns: BoardPattern[] = Array.from({ length: 5 }, (_, index) => ({
  ...
  const linkedEntities: Record<string, BoardLinkedEntity> = {};
  ...
  const threads: BoardThread[] = Array.from({ length: 95 }, (_, index) => {
    ...
    return {
      ...
      hook: threadNumber % 3 === 0 ? `Pressure note ${String(threadNumber).padStart(2, '0')} needs attention.` : undefined,
  ...
  return {
    ...
    now: {
      ...
      playerSummary: 'This seeded campaign is built to stress-test note density, hover states, and cross-pattern links.',
      gmTruth: 'Five pattern notes each own eight threats, and the remaining threats create extra random cross-pattern pressure.',
    },
    threads,
    patterns,
    linkedEntities,
    ...
    ] satisfies BoardPlayerProfile[],
  };
}

const BOARD_PROJECTS: Record<string, ProjectBoardData> = {
  'project-1': {
    project: {
      id: 'project-1',
    ...
    now: {
      id: 'now',
      title: 'Current moment',
      playerSummary: 'The harbor is uneasy, the watch is stretched thin, and too many rumors point toward the old ruins.',
      gmTruth:
    ...
    patterns: [
      {
        ...
        summary: 'The docks are being manipulated so disappearances look like labor unrest instead of coordinated pressure.',
        escalationLevel: 2,
      ...
      },
      {
        ...
        summary: 'The chapel fire, relic theft, and strange ash all point to ritual preparation close to the city center.',
        escalationLevel: 3,
      ...
      },
      {
        ...
        summary: 'A hidden courier network is moving coded orders under the cover of dockside chaos.',
        escalationLevel: 2,
      ...
      },
      {
        ...
        summary: 'Lantern codes, safe approaches, and trap routes are quietly shaping who reaches the ruins alive.',
        escalationLevel: 1,
    ...
    threads: [
      ...
      },
      {
        ...
        playerSummary: 'The watch lost contact with an entire patrol near the low-tide tunnels.',
      ...
      },
      {
        ...
        state: 'resolved',
        playerSummary: 'A labor pledge briefly stabilized the docks before fear took over again.',
      ...
      },
      {
        ...
        playerSummary: 'The chapel remains closed after a suspicious fire and a handful of whispered sightings.',
      ...
      },
      {
        ...
        gmTruth: 'The stolen relic is the final focus item for the Cinder Choir’s harbor rite.',
      ...
      },
      {
        ...
        gmTruth: 'The ledger reveals how the cult’s harbor cells coordinate every public disruption.',
        timelineAnchor: 'now',
      ...
      },
      {
        ...
        state: 'active',
        playerSummary: 'A broker warns that blue lanterns mark safe routes and amber lanterns mark traps.',
        gmTruth: 'Sera is feeding selective truths in exchange for immunity after the crackdown.',
        timelineAnchor: 'future_possible',
  ...
  'project-2': {
    ...
    now: {
      id: 'now',
      title: 'Current moment',
      playerSummary: 'Signal towers are flickering out of sync, and every village is blaming a different enemy.',
      gmTruth:
    ...
    patterns: [
      {
        ...
        summary: 'Signal towers are failing in a deliberate pattern, creating panic and tactical blind spots.',
        escalationLevel: 2,
      ...
      },
      {
        ...
        title: 'Buried Engine',
        summary: 'Something ancient under Signal Hill is learning the region’s wartime habits and exploiting them.',
        escalationLevel: 3,
      ...
      },
      {
        ...
        title: 'River Static',
        summary: 'Ferry crews and messengers spread conflicting signal warnings before any tower can confirm them.',
        escalationLevel: 1,
    ...
    threads: [
      {
        ...
        state: 'active',
        playerSummary: 'One tower keeps flashing a red pattern no operator admits to sending.',
      ...
      },
      {
        ...
        gmTruth: 'The engine is waking faster every time the network carries a false alarm.',
        timelineAnchor: 'now',
      ...
      },
      {
        ...
        title: 'River Static',
        state: 'active',
        playerSummary: 'Boat crews repeat warning phrases before any official signal reaches the river crossing.',
        gmTruth: 'The buried engine is piggybacking on ferry gossip to amplify false mobilization orders.',
        timelineAnchor: 'past',
  ...
  'project-3': createHundredNotesTestProject(),
};

export const BOARD_PROJECT_OPTIONS: BoardProjectOption[] = Object.values(BOARD_PROJECTS).map((project) => ({
  id: project.project.id,
  name: project.project.name,
}));

export function getProjectBoardData(projectId: string): ProjectBoardData {
  return BOARD_PROJECTS[projectId] ?? BOARD_PROJECTS['project-1'];
}
