...
function sentenceForNoThreads(role: UserRole) {
  ...
  if (role === 'helper') return 'Coordinate with your GM by drafting a support thread the players can discover.';
...
function sentenceForNoTimelineEvents(role: UserRole) {
  ...
  if (role === 'helper') return 'Add a supporting timeline beat to reinforce the opening situation.';
...
function sentenceForFetchFailure(role: UserRole) {
  ...
  if (role === 'helper') return 'We could not sync shared campaign data; retry and continue assisting your GM.';
